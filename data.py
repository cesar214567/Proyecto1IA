...
with open('data_parsed.csv','w+') as csv_file:
    writer = csv.writer(csv_file, delimiter=',', quotechar='"', quoting=csv.QUOTE_MINIMAL)
    writer.writerow(['date','region','region_orig','deaths','confirmed'])
    for index,row in data.iterrows():
        writer.writerow([get_date_differential(row['date']),row['region'],row['region_orig'],row['deaths'],row['confirmed']])
...
#    for index,row in data.iterrows():
#        if(zonas[row['zone']] == 0 and row['lon']!=-77.0308 and row['lat']!=-12.0453 ):
